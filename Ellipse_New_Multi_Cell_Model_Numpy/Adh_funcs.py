...
def random_adhesions(a, b, cells, cparams,
    ...
    for ind in range(cells.shape[0]//3):
        ...
        x_max = np.ceil(max(cells[3*ind] + cparams[4*ind]*cos(cparams[4*ind+2]), 
                     cells[3*ind] - cparams[4*ind]*cos(cparams[4*ind+2])))
        x_min = np.floor(min(cells[3*ind] + cparams[4*ind]*cos(cparams[4*ind+2]), 
                     cells[3*ind] - cparams[4*ind]*cos(cparams[4*ind+2])))                
        y_max = np.ceil(max(cells[3*ind+1] + cparams[4*ind+1]*cos(cparams[4*ind+2]), 
                     cells[3*ind+1] - cparams[4*ind+1]*cos(cparams[4*ind+2])))
        y_min = np.floor(min(cells[3*ind+1] + cparams[4*ind+1]*cos(cparams[4*ind+2]), 
                     cells[3*ind+1] - cparams[4*ind+1]*cos(cparams[4*ind+2])))  
        num_x = (x_max - x_min)*4
        num_y = (y_max - y_min)*4
        ...
        for ad in adh_ind:
            ...
            while found == 0:
                x = x_min + 0.25*uniform_ints(0, num_x, 1)[0]
                y = y_min + 0.25*uniform_ints(0, num_y, 1)[0]
