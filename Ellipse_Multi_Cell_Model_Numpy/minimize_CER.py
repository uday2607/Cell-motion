...
#From: https://github.com/linesd/minimize/blob/master/optimizer/minimize.py
@nb.jit(nopython = True, nogil = True)
def minimize(f, X, args=(), length=1e4, reduction=None, concise=False):
	"""
	...
	f0, df0 = f(X, *args)			# get initial function value and gradient
	fX=np.empty(0); fX=np.append(fX, f0)
	Xd=np.empty(0); Xd=np.append(Xd, X)
